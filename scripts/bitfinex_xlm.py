...
def on_message(mes):
    ...
    print("Bitfinex - 9 - XLM-USD:" + " " + mes)
    message = json.loads(mes)
    if message[1] in ('tu'):
        result = {'_id': message[2][0], 't': message[2][1], 'q': message[2][2], 'p': message[2][3],
                  's': 'XLM-USD', 'date': datetime.datetime.utcnow().strftime('%Y-%m-%d')}
        res = bitfinex_coll.insert_one(result)


ws = websocket.WebSocketApp('wss://api-pub.bitfinex.com/ws/2')

ws.on_open = lambda self: self.send('{ "event": "subscribe", "channel": "trades", "symbol": "tXLMUSD"}')
